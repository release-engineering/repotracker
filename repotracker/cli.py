...
def get_args():
    parser = argparse.ArgumentParser(
        description="Send a message when the contents of a container repository changes"
    )
    parser.add_argument("-q", "--quiet", help="Suppress output", action="store_true")
    parser.add_argument(
        "-v", "--verbose", help="Print extended output", action="store_true"
    )
    parser.add_argument(
        "-c", "--config", help="Config file", default="/etc/repotracker/repotracker.ini"
    )
    parser.add_argument(
        "-d",
        "--data",
